...
Всё управляется через интерактивное меню в терминале. 19 модулей, ноль внешних зависимостей кроме bash.
...
Ну конечно же для энтузиастов, для учёбы, работы. Сугубо в личных целях:
...
| Требование | Значение |
|------------|---------|
| ОС | Debian 12 / 13 (Ubuntu 22.04 / 24.04 -> частичная поддержка) |
...
| Компонент | Что делает | Для кого |
|-----------|-----------|----------|
| **AmneziaWG** | Шифрует и маскирует весь трафик. Поддерживает AWG 1.0, AWG 2.0 и обычный WireGuard. Мультиинтерфейс, QR-коды для мобильных клиентов | Основной VPN для ежедневного использования |
...
| Компонент | Что делает | Для кого |
|-----------|-----------|----------|
| **MTProto** | Прокси для Telegram с маскировкой (Fake TLS). Генерирует ссылку -> нажал, подключился. Мультиинстанс | Telegram в заблокированных регионах |
| **SOCKS5** | Универсальный прокси с логином и паролем. Работает с любым приложением | Браузеры, Telegram, любые приложения |
| **Hysteria 2** | Быстрый прокси на базе QUIC/UDP. Хорошо работает на каналах с потерями пакетов | Быстрый обход блокировок |
...
| **AWG 2.0** | Jc, Jmin, Jmax, S1-S4, H1-H4 (диапазоны min-max) | Keenetic 5.1+, AmneziaVPN 4.8.12.9+, OpenWrt с пакетами AWG 2.0 |
| **WireGuard** | Всё обнулено, H1-H4 = 1-4 | Любой стандартный WireGuard клиент |

AWG 2.0 добавляет padding для cookie (S3) и data (S4) пакетов, а H1-H4 генерируются как непересекающиеся диапазоны в разных порядках величины -> DPI не может зацепиться за статичный header.
...
Клиентские конфиги содержат Keenetic `asc` команду (AWG 1.0) или предупреждение (2.0/WG). QR-код выводится в терминале -> навёл камеру, подключился.

## MTProto -> подробности

Docker-контейнер `seriyps/mtproto-proxy` с режимом Fake TLS (`-a tls`). Маскируется под TLS-трафик к указанному домену (видно в SNI). Генерирует ссылку `tg://proxy` с `ee`-префиксом (Fake TLS).
...
Если VPS целиком лёг -> бот не пришлёт (он на том же VPS). Для внешнего мониторинга -> (https://uptimerobot.com).
...
Архивы в `/root/eli-backups/`. При восстановлении: распаковка → стоп сервисов → раскладка файлов → старт → отчёт.
...
├── README.md
├── eli.sh                      собранный скрипт (~8000 строк)
└── src/
...
10. **Telegram бот**: мониторит только изнутри VPS. Если VPS целиком лёг то бот не работает. Для внешнего мониторинга: (https://uptimerobot.com).
...
12. **Diag exec redirect**: `exec > >(tee)` для TXT отчёта, очистка через `trap RETURN` (без двойного закрытия FD).
...
- Мы получали запрос конвертировать скрипт под Ubuntu, но чёта пока лень... и времени нет... и отдавать ИИшке на конверт тоже не будем... и 200 бачей платить не будем, этоже месячный запас шаурмы!
